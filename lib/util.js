...
module.exports = {
  ...
  */
   ...
   */
  convertPathVariables: function(type, providedPathVars, commonPathVars, bodyTypeOption) {
      ...
      _.forEach(commonPathVars, (variable) => {
        variables.push({
          ...
          value: this.options.schemaFaker ?
            safeSchemaFaker(variable.schema || {}, bodyTypeOption, this.components, SCHEMA_FORMATS.DEFAULT) : '',
  ...
  convertToPmQueryParameters: function(param) {
    ...
    if (param.hasOwnProperty('schema')) {
      ...
      paramValue = this.options.schemaFaker ?
        safeSchemaFaker(param.schema, BODY_TYPE.REQUEST, this.components, SCHEMA_FORMATS.DEFAULT) : '';
   ...
   */
  convertToPmHeader: function(header, bodyTypeOption) {
    var fakeData,
      reqHeader;
    ...
    if (header.hasOwnProperty('schema')) {
      if (!this.options.schemaFaker) {
        ...
        fakeData = safeSchemaFaker(header.schema || {}, bodyTypeOption, this.components, SCHEMA_FORMATS.DEFAULT);
        ...
        if (encoding.hasOwnProperty(key)) {
          _.forOwn(encoding[key].headers, (value, key) => {
            if (key !== 'Content-Type') {
              ...
              formHeaders.push(this.convertToPmHeader(encoding[key].headers[key]), BODY_TYPE.REQUEST);
            }
          });
        }
        if (typeof value === 'object') { value = JSON.stringify(value); }
  ...
  convertToPmResponse: function(response, code, originalRequest) {
    ...
    _.forOwn(response.headers, (value, key) => {
      if (key !== 'Content-Type') {
        ...
        header.name = key;
        responseHeaders.push(this.convertToPmHeader(header, BODY_TYPE.RESPONSE));
      }
    });
  ...
  convertRequestToItem: function(openapi, operationItem) {
    ...
    if (Array.isArray(localServers) && localServers.length) {
      ...
      if (serverObj.variables) {
        pathVarArray = this.convertPathVariables('method', [], serverObj.variables, BODY_TYPE.REQUEST);
      }
    }
    else {
      ...
      pathVarArray = this.convertPathVariables('root', [], pathVariables, BODY_TYPE.REQUEST);
    }
    ...
    // TODO: Figure out what happens if type!=api-key
      ...
      if (authHelper.properties.in === 'header') {
        item.request.addHeader(this.convertToPmHeader(authHelper.properties, BODY_TYPE.REQUEST));
        item.request.auth = {
          type: 'noauth'
        };
      }
    ...
    _.forEach(reqParams.header, (header) => {
      item.request.addHeader(this.convertToPmHeader(header, BODY_TYPE.REQUEST));
    });
